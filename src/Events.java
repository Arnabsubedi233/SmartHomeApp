...
final class AutoSaveObserver implements Observer {
    ...
    public void update(SystemEvent event) {
        ...
        boolean shouldSave =
                event.getType() == EventType.SENSOR_ADDED ||
                event.getType() == EventType.SENSOR_TOGGLED ||
                event.getType() == EventType.CONTACTS_UPDATED ||
                event.getType() == EventType.ALARM_CONFIG_UPDATED ||
                event.getType() == EventType.SYSTEM_ARMED ||
                event.getType() == EventType.SYSTEM_DISARMED;
        ...
        try {
            ...
            log.error("Auto-save failure: " + e.getMessage());
        }
    }
}
